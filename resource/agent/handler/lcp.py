...
class LCP(BaseLCP):
    ...
    def handler(cls, instance, req, resp):
        ...
        if all([agent_catalog, exec_env]):
            ...
            return lcp.__apply(instance=instance, exec_env=exec_env)
        return False
            ...
            if resp_lcp.content:
                try:
                    ...
                    if resp_lcp.status_code >= 300 or (
                        is_dict(resp_lcp) and resp_lcp_data.get("error", False)
                    ):
                        UnprocEntityResponse(resp_lcp_data).add(self.resp)
                        return False
                    else:
