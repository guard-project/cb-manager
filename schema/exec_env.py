...
class LCPSchema(Schema):
    ...
    port = Integer(required=True, example=5000,
                   description='TCP port number of LCP in the execution environment.')  # noqa: E501
    https = Boolean(required=True, default=False, example=True,
    ...
    started = DateTime(readonly=True, example='2019/02/14 15:23:30',
                       description='Timestamp when the LCP is started.')
    last_heartbeat = DateTime(reaonly=True, example='2019/02/14 15:23:33',
...
class ExecEnvSchema(BaseSchema):
    ...
    lcp = Nested(LCPSchema, readonly=True, unknown='INCLUDE',
                 description='Data related to the LCP.')
    description = Str(example='Apache HTTP Web Server.',
                      description='Short description of the execution environment,')  # noqa: E501
    enabled = Boolean(required=True, default=True, example='Yes',
                      description='Indicate if the execution environment is enabled or not')  # noqa: E501
