...
class AgentInstanceActionSchema(Schema):
    ...
    output_format = Str(enum=OUTPUT_FORMATS, example=OUTPUT_FORMATS[1],
                        default=OUTPUT_FORMATS[0],
                        description='Output format for stdout and stderr result of action execution.')  # noqa:E501
...
class AgentInstanceOperationSchema(BaseSchema):
    ...
    actions = Nested(AgentInstanceActionSchema, many=True, unknown='INCLUDE',
    ...
    resources = Nested(AgentInstanceResourceSchema, many=True,
                       unknown='INCLUDE',
...
class AgentInstanceSchema(BaseSchema):
    ...
    id = Str(required=True, example='filebeat@apache',
             description='Id of the agent instance installed in an execution environment.')  # noqa: E501
    ...
    exec_env_id = Str(required=True, readonly=True, example='apache',
                      ...
                      error_messages=In.error_messages)
    status = Str(enum=AGENT_STATUS, required=True, readonly=True,
                 example=AGENT_STATUS[0],
                 description='Status of the agent.')
    operations = Nested(AgentInstanceOperationSchema, many=True,
                        unknown='INCLUDE',
