...
class AgentCatalogActionSchema(Schema):
    ...
    # TODO unique?
    config = Nested(AgentCatalogActionConfigSchema, unknown='INCLUDE',
                    required=True, description='Action config.')
...
class AgentCatalogParameterConfigSchema(Schema):
    ...
    schema = Str(required=True, enum=PARAMETER_SCHEMAS,
    ...
    path = ListOrOne(Str, required=True, example='enabled',
...
class AgentCatalogParameterSchema(Schema):
    ...
    config = Nested(AgentCatalogParameterConfigSchema, unknown='INCLUDE',
                    required=True, description='Parameter configuration.')
    list = Bool(default=False, example=True,
                description='Indicate if the parameter can have multiple values.')  # noqa: E501
    values = ListOrOne(Str, example='mysql',
...
class AgentCatalogResourceConfigSchema(Schema):
    ...
    path = ListOrOne(Str, required=True,
...
class AgentCatalogResourceSchema(Schema):
    ...
    config = Nested(AgentCatalogResourceConfigSchema, unknown='INCLUDE',
                    required=True, description='Resource configuration.')
...
class AgentCatalogSchema(BaseSchema):
    ...
    actions = Nested(AgentCatalogActionSchema, unknown='INCLUDE', many=True,
                     description='Action properties.',
                     validate=UniqueList.apply('id'),
                     error_messages=UniqueList.error_messages)
    parameters = Nested(AgentCatalogParameterSchema, unknown='INCLUDE',
                        many=True, description='Parameter properties.',
                        validate=UniqueList.apply('id'),
                        error_messages=UniqueList.error_messages)
    resources = Nested(AgentCatalogResourceSchema, unknown='INCLUDE',
                       many=True, description='Resource properties.',
                       validate=UniqueList.apply('id'),
                       error_messages=UniqueList.error_messages)
    description = Str(example='Collect system metrics from execution environments.',  # noqa: E501
                      description='Short description of the agent.')
    deployment_source = ListOrOne(Str, example='https://www.example.com/k8s-artefact.yaml',  # noqa: E501
