...
class _eBPFProgramCatalogConfigMetricOpenMetricsMetadataSchema(Schema):
    ...
    help = Str(
        example="This metric represents the number of packets that has travelled trough this probe.",  # noqa: E501
        description="Metric help.",
    )
    labels = Nested(
        _eBPFProgramCatalogConfigMetricOpenMetricsMetadataLabelSchema,  # noqa: E501
        many=True,
        unknown="INCLUDE",
...
class _eBPFProgramCatalogConfigMetricSchema(Schema):
    ...
    name = Str(
        required=True, example="packets_total", description="Metric name."
    )
    map_name = Str(
        required=True,
        example="PKT_COUNTER",
    ...
    open_metrics_metadata = Nested(
        _eBPFProgramCatalogConfigMetricOpenMetricsMetadataSchema,  # noqa: E501
...
class _eBPFProgramCatalogConfigSchema(Schema):
    ...
    code = ListOrOne(
        Str, required=True, description="Code of the eBPF program."
    )
    metrics = Nested(
...
class _eBPFProgramCatalogSchema(BaseSchema):
    ...
    config = Nested(
        _eBPFProgramCatalogConfigSchema, required=True, unknown="INCLUDE"
    )
    parameters = Nested(
    ...
    description = Str(
        example="Transparent service to capture packets flowing through the interface it is attached to, apply filters and obtain capture in .pcap format.",  # noqa: E501
